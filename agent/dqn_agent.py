...
PRIORITY_WEIGHT_BETA = 0.5  # Exponent for the priority importance loss scaling
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.array([e.state for e in sample])).float().to(device)
        ...
        next_states = torch.from_numpy(np.array([e.next_state for e in sample])).float().to(device)
